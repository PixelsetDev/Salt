...
export const SuccessBox = ({ message }: { message: string; }) => {
...
export const Box = ({ message, background, border, icon }: { message: string; background: string; border: string; icon: string; }) => {
  return(
    <View className={`flex-row ${background}`}>
      <FontAwesome name={icon} size={20} className={`text-white px-2 py-1 ${border} pt-2`}/><OText className={`text-white px-2 py-1`}>&nbsp;{message}</OText>
    </View>
  );
}